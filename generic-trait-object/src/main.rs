...
/// 'static is the only one that should be included in this pattern.
trait Element: 'static + Debug {}

/// Our target.
trait Generic {
    fn generic_writes<E: Element>(&mut self, fn_table: Option<&FnTable>, input: &mut E);
    fn generic_reads<E: Element>(&mut self, fn_table: Option<&FnTable>, output: &mut E);
}

/// Generic erased.
/// We're going to make a trait object based on this.
trait ErasedGeneric {
    fn erased(&mut self, fn_table: Option<&FnTable>, input: &mut dyn Any);
}
...
impl Generic for dyn ErasedGeneric {
    #[inline]
    fn generic_writes<E: Element>(&mut self, fn_table: Option<&FnTable>, input: &mut E) {
        self.erased(fn_table, input as &mut dyn Any);
    }

    #[inline]
    fn generic_reads<E: Element>(&mut self, fn_table: Option<&FnTable>, output: &mut E) {
        self.erased(fn_table, output as &mut dyn Any);
    }
}
...
impl ErasedGeneric for Handler {
    #[inline]
    fn erased(&mut self, fn_table: Option<&FnTable>, input: &mut dyn Any) {
        // Acquires `TypeId` from the `dyn Any` and gets an appropriate delegator.
        let ty_id = (input as &dyn Any).type_id();
        let delegator = fn_table
            .expect("fn_table must be filled.")
            .get(&ty_id)
            .expect("fn_table doesn't have appropriate entry.");

        // `delegator' calls the real generic method in it.
        (delegator)(self, input);
    }
}

/// A struct for implementing on the trait `Target`.
struct Handler {
...
impl Generic for Handler {
    ...
    fn generic_reads<E: Element>(&mut self, _fn_table: Option<&FnTable>, output: &mut E) {
    ...
    ($fn_table:ident, $handler:ty, $generic_ty:ty, $generic_fn:ident) => {
        $fn_table.insert(
            ...
            Box::new(|handler: &mut $handler, value: &mut dyn Any| {
                handler.$generic_fn(None, value.downcast_mut::<$generic_ty>().unwrap());
            }),
        );
    };
}
...
fn main() {
    ...
    // Writes something.
    trait_object.generic_writes(Some(&fn_table_writes), &mut A { _a1: 1, _a2: 2 });
    trait_object.generic_writes(Some(&fn_table_writes), &mut B { _b1: 3 });

    // Reads back.
    let mut a_read = A { _a1: 0, _a2: 0 };
    let mut b_read = B { _b1: 0 };
    trait_object.generic_reads(Some(&fn_table_reads), &mut b_read);
    trait_object.generic_reads(Some(&fn_table_reads), &mut a_read);
